    ...
    def create_telebot(self):
        ...
        def send_welcome(message):
            ...
            msg = bot.send_message(
                message.chat.id,
                text="Where are you flying (Country/Airport code) ?",
                reply_markup=markup,
            )
        ...
        def parse_country(message):
            self.chats[message.chat.id]["country"] = (
                message.text if message.text != "any" else ""
            )
            ...
            bot.send_message(message.chat.id, text="Select Year", reply_markup=calendar)
        ...
        def parse_date_to(message, result):
            ...
            msg = bot.send_message(
                message.chat.id,
                text="Min number of nights in destination:",
                reply_markup=markup,
            )
        ...
        def parse_min_nights(message):
            ...
            msg = bot.send_message(
                message.chat.id,
                text="Max number of nights in destination:",
                reply_markup=markup,
            )
        ...
        def parse_max_nights(message):
            ...
            markup.add(
                telebot.types.KeyboardButton("yes"), telebot.types.KeyboardButton("no")
            )
            msg = bot.send_message(
                message.chat.id, text="Fly only weekends?", reply_markup=markup
            )
        ...
        def parse_only_weekends(message):
            ...
            msg = bot.send_message(
                message.chat.id, text="Max price:", reply_markup=markup
            )
        ...
        def get_query_values_from_db(chat_id):
            query_values = db.UserQueryDetails.select().where(
                db.UserQueryDetails.chat_id == chat_id
            )
        ...
        def scan(message):
            if message.chat.id in self.chats:
                ...
                db.UserQueryDetails.delete().where(
                    db.UserQueryDetails.chat_id == message.chat.id
                ).execute()
                db.UserQueryDetails(
                    fly_from="IL",
                ...
                if not flight_details:
                    bot.send_message(
                        message.chat.id, "No config is found, please type /start :)"
                    )
                    return
            ...
            bot.send_message(
                message.chat.id, "You can /rescan with current values, or /start again"
            )
