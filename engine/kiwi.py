...
def generate_holidays_flights(
    ...
    fly_to: str,
    price_to: int = 1000,
    nights_in_dst_from: str = 1,
    nights_in_dst_to: str = 10,
    country="IL",
    scan_timestamp=None,
) -> int:
...
def prepare_kiwi_api(
    ...
    kiwi_api_params["date_from"] = date_from.strftime("%Y-%m-%d")
    kiwi_api_params["return_from"] = date_from.strftime("%Y-%m-%d")
    kiwi_api_params["date_to"] = date_to.strftime("%Y-%m-%d")
    kiwi_api_params["return_to"] = date_to.strftime("%Y-%m-%d")
...
def generate_flights(
    ...
    for flight in flights_data:
        ...
        source = (
            f"{flight['countryFrom']['name']}/{flight['cityFrom']}/{flight['flyFrom']}"
        )
        ...
        if len(set(airlines_list)) == 1:
            if airline_class:
                links[0] = links[1] = airline_class.generate_link(
                    flight["flyFrom"],
                    flight["flyTo"],
        ...
        db_flight = db.Flights(
            ...
            nights=int(flight["nightsInDest"]),
